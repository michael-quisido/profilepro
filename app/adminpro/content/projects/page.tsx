    ...
    async function loadProjects() {
      try {
        ...
        if (data.projects && data.projects.length > 0) {
          setContent(data.projects.map((p: any) => p.name));
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    try {
      await fetch("/api/db/projects", {
        ...
        body: JSON.stringify({ projects: content }),
      });
      setSaved(true);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f5" }}>
      ...
      <div style={{ padding: "40px", maxWidth: "1000px", margin: "0 auto" }}>
        <h2 style={{ marginBottom: "10px", color: "#333" }}>Edit Projects Page</h2>
        <div style={{ display: 'flex', gap: '20px', marginBottom: '30px' }}>
          <Link href="/adminpro/content/projects" style={{ padding: '8px 16px', background: '#c9a227', color: 'white', borderRadius: '5px', textDecoration: 'none' }}>Projects</Link>
          <Link href="/adminpro/content/about" style={{ padding: '8px 16px', background: '#666', color: 'white', borderRadius: '5px', textDecoration: 'none' }}>About</Link>
        ...
        <div style={{ background: "white", padding: "30px", borderRadius: "15px", marginBottom: "30px" }}>
          <h3 style={{ marginBottom: "20px", color: "#333" }}>Projects</h3>
          <RichTextEditor
            value={content.join('<br/>')}
            onChange={(value) => setContent(value.split('<br/>').filter(p => p.trim()))}
          />
        </div>
